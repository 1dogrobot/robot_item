...
from visualization_msgs.msg import Marker,MarkerArray #markerarry can solve moti node
...
def publish_ego_car(ego_car_pub):
    ...
    marker.type = Marker.LINE_STRIP  #liner
    # line
    marker.color.r = 0.0
    marker.color.g = 1.0
    marker.color.b = 0.0
    ...
    mesh_marker.mesh_resource = "package://kitti_test/aaa/Car-Model/Car.dae"  #LOAD ERROR, DON'T KNOW WHY
    ...
    '''use markerarrat to solve multi marker node'''

def publish_imu(imu_pub, imu_data):
    imu=Imu()   
    imu.header.frame_id = FRAME_ID
    imu.header.stamp = rospy.Time.now()
    q = tf.transformations.quaternion_from_euler(float(imu_data.roll), float(imu_data.pitch),float(imu_data.yaw)) # prevent the data from being overwritten
    '''xuanzhuann '''
